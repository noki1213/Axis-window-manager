...
struct GapInfo: Equatable {
	...
	let columnIndex: Int
	let rowIndex: Int? // Used only when horizontal
...
class GapSelectManager: ObservableObject {
	...
	func startResizeGapInDirection(_ direction: Direction) -> Bool {
		...
		for (index, gap) in availableGaps.enumerated() {
			...
			case .up: // Upward on screen (smaller row index)
			...
			case .down: // Downward on screen (larger row index)
		...
		if let index = targetGapIndex {
			...
			// Show the overlay
			showOverlay(on: screen)
			BorderManager.shared.updateBorder() // Remove the border
			return true
		}
