...
class HotkeyManager: ObservableObject {
    ...
    private func handleKeyEvent(_ event: NSEvent) -> Bool {
        ...
        case kVK_ANSI_S: // Operate on the left gap
            DispatchQueue.main.async { [weak self] in
        ...
        case kVK_ANSI_F: // Operate on the right gap
            DispatchQueue.main.async { [weak self] in
        ...
        case kVK_ANSI_E: // Operate on the top gap
            DispatchQueue.main.async { [weak self] in
        ...
        case kVK_ANSI_D: // Operate on the bottom gap
            DispatchQueue.main.async { [weak self] in
        ...
        // MARK: Mode Switching
        case kVK_ANSI_W: // Window selection mode
            DispatchQueue.main.async { [weak self] in
        ...
        case kVK_ANSI_G: // Gap selection mode
            DispatchQueue.main.async { [weak self] in
                self?.currentMode = .gapSelect
        ...
        case kVK_ANSI_P: // Window switcher mode
            DispatchQueue.main.async { [weak self] in
        ...
        // MARK: Reset Layout
        case kVK_ANSI_R: // Reset all windows back to a vertical split
            DispatchQueue.main.async { [weak self] in
        ...
        case kVK_ANSI_M, kVK_ANSI_Q: // Move cursor between monitors
            DispatchQueue.main.async { [weak self] in
        ...
        case kVK_ANSI_O: // Next workspace (+1) / move with Shift
            DispatchQueue.main.async {
        ...
        case kVK_ANSI_U: // Previous workspace (-1) / move with Shift
            DispatchQueue.main.async {
                guard let screen = WorkspaceManager.shared.focusedScreen() else { return }
                if hasShift {
